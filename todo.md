...
- [ ] 마이페이지 작품 업로드 버튼 가시성 개선 (드래그&드롭 + 큰 버튼)
- [ ] DB 스키마 — exhibitions 테이블 추가 (전시회 이름, 슬러그, 인원 설정, 기간 등)
- [ ] 관리자: 전시회 생성/수정/삭제 UI
- [ ] 전시회별 참여 인원 자유 설정 (최소 1명~무제한)
- [ ] 전시회 선택 랜딩 페이지 (여러 전시회 목록)
- [ ] 전시회별 독립 URL 지원 (/exhibition/:slug)
- [ ] 전시회와 작가/작품 연결 구조 (artist.exhibitionId)
